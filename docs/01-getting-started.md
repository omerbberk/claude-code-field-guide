...
**Requirements:** macOS, Linux, or Windows (native or WSL), and Node.js 18+ for the npm install path.

```bash
# npm (most common)
npm install -g @anthropic-ai/claude-code

# macOS/Linux native installer (no Node required)
...
Claude Code keeps itself up to date automatically. You can trigger an update manually with `claude update`.

## Authentication

Run `claude` in any folder. On first launch you pick one of:

1. **Claude subscription (Pro or Max)** — log in with your claude.ai account. Usage is included in your plan. This is the simplest option for individuals.
...
| `Esc Esc` | Jump back in history / rewind the conversation |
...
| `/cost` | Show token usage for the session (API billing) |
